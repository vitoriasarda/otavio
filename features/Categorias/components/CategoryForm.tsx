...
interface CategoryFormProps {
  ...
  onUpdateCategory?: (id: number, category: NewCategory) => void;
  onCancelEdit?: () => void;
}
...
export function CategoryForm({
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (isEditing && editingCategory && onUpdateCategory) {
      onUpdateCategory(editingCategory.id, { description });
    } else {
      onAddCategory({ description });
    }
    setDescription("");
  };
  ...
  return (
    <div className="w-full bg-white rounded-lg p-6 md:p-8">
      ...
      >
        ...
        <div className="mt-6 md:mt-0 flex justify-end gap-4">
          <Button
            type="submit"
          ...
          >
            {isEditing ? "CONFIRMAR" : "ADICIONAR CATEGORIA"}
          </Button>
          {isEditing && (
            <Button
              type="button"
