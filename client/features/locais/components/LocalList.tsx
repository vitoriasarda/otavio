...
export function LocalList({
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredLocais.length / ITEMS_PER_PAGE),
  );
    ...
    <div className="w-full bg-white rounded-lg p-6 md:p-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div className="relative flex items-center w-full md:w-[406px]">
          ...
          <Input
            type="text"
            placeholder="PESQUISAR EM LOCAIS CADASTRADOS"
            value={searchTerm}
      ...
      <div className="overflow-x-auto mb-6">
        <Table className="w-full table-fixed">
          <TableHeader>
            <TableRow>
              <TableHead className="text-[16px] font-medium text-primary-blue w-[40%] text-left">
                DESCRIÇÃO
              </TableHead>
              <TableHead className="text-[16px] font-medium text-primary-blue w-[40%] text-left">
                ENDEREÇO/LOCALIZAÇÃO
              </TableHead>
              <TableHead className="text-[16px] font-medium text-primary-blue w-[20%] text-left">
                AÇÕES
              </TableHead>
            </TableRow>
          </TableHeader>
            ...
            {paginatedLocais.length > 0 ? (
              paginatedLocais.map((local) => (
                <TableRow key={local.id}>
                  <TableCell className="text-[14px] truncate text-left">
                    {local.description}
                  </TableCell>
                  <TableCell className="text-[14px] truncate text-left">
                    {local.address}
